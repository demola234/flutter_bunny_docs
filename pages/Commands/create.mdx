...
1. [Generate components](generate) with the `generate` command
2. [Configure your project settings](config) with the `config` command
3. [Run static analysis](analyze) with the `analyze` command

<Callout>
  Looking for more details on architecture? Check out the [Architecture Guide](/architectural_structure) for in-depth information on each architecture pattern.
</Callout>